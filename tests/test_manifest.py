...
class TestReportingFunctions:
    ...
    def test_print_failures_only_not_needed(self, monkeypatch: pytest.MonkeyPatch) -> None:
        con, buf = self._make_console()
        monkeypatch.setattr("scripts.manifest._console", con)
        mark_not_needed("system/bpf/0001-foo.patch")
        print_failures()
    ...
    def test_print_failures_both_categories(self, monkeypatch: pytest.MonkeyPatch) -> None:
        con, buf = self._make_console()
        monkeypatch.setattr("scripts.manifest._console", con)
